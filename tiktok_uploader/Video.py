...
class Video:
    ...
    def get_youtube_video(self, max_res=True, max_retries=3, timeout=60):
        url = self.source_ref
        
        for attempt in range(max_retries):
            try:
                # Configure YouTube with longer timeout
                yt = YouTube(url, use_oauth=False, allow_oauth_cache=False)
                yt.streaming_data['timeout'] = timeout  # Increase streaming timeout
                ...
                if filtered_streams:
                    ...
                    print(f"Starting Download for Video... (Attempt {attempt + 1}/{max_retries})")
                    print(f"Selected quality: {selected_stream.resolution}")
                    
                    # Download with progress callback
                    selected_stream.download(
                        output_path=os.path.join(os.getcwd(), Config.get().videos_dir),
                        filename="pre-processed.mp4"
                    )
                ...
                print(f"Download failed on attempt {attempt + 1}, retrying immediately...")
                # No delay - retry immediately
                
            except Exception as e:
                print(f"Error during download (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    print("Retrying download immediately...")
                    # No delay - retry immediately
                else:
            ...
            if resolution >= 360:
                ...
                while not os.path.exists(downloaded_a_path) and os.path.exists(downloaded_v_path):
                    ...
                    if file_check_iter > 3:
                        ...
                        return
                    print("Checking for files to appear.")
